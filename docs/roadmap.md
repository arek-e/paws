...
**Goal:** A working system on one Linux box. Submit workloads, run in isolated Firecracker VMs, zero secrets in the sandbox.
...
**v0.1 deliverable:** `bun run start` on a Hetzner server. Hit the API, workloads run in isolated VMs, credentials never enter the sandbox.

**Testing in v0.1:** Test-first for types, scheduler, ip-pool, client, proxy logic, gateway routes. Test-after for system plumbing. See [testing.md](testing.md).
...
**v0.2 deliverable:** `kubectl apply` deploys paws. Two worker nodes, gateway routes sessions to least-loaded.
