...
Your Tailscale identity must be authorized in the tailnet ACLs. The user `alex` is **not** permitted — use `root`.
...
1. **Restore from snapshot** — `/var/lib/firecracker/snapshots/agent-latest/` (disk.ext4 4GB + memory.snap 4GB + vmstate.snap 30KB)
