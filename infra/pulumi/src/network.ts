 ...
 *  - A firewall that:
 *      • allows SSH from anywhere (Tailscale should be added in prod)
...
export function createNetwork(): NetworkOutputs {
  ...
  const clusterFirewall = new hcloud.Firewall('paws-firewall', {
    ...
    rules: [
      // SSH — restrict to your own CIDR in production; open here for bootstrapping.
      {
        direction: 'in',
        protocol: 'tcp',
        port: '22',
        sourceIps: ['0.0.0.0/0', '::/0'],
        description: 'SSH access for bootstrap',
      },
...
export type { pulumi as _pulumiType }; // keep import used
