...
describe('createPangolinResourceManager', () => {
  ...
  describe('expose', () => {
    it('creates Pangolin resources for each port', async () => {
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({ data: { resourceId: 'res-1' } }), { status: 201 }),
      );
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({ data: { resourceId: 'res-2' } }), { status: 201 }),
      );
      ...
      expect(url1).toBe('http://pangolin:3001/api/v1/org/org-123/site/site-456/resource');
      const body1 = JSON.parse((opts1 as RequestInit).body as string);
      expect(body1.subdomain).toBe('s-abcdef12-3000');
      expect(body1.target).toBe(10001);
    });
  ...
  describe('cleanup', () => {
    it('deletes each resource', async () => {
      ...
      expect(url1).toBe('http://pangolin:3001/api/v1/org/org-123/site/site-456/resource/res-1');
